...
class Product {
    ...
    public function updateProduct($id_producto, $nombre, $precio, $descripcion, $imagenFile){
        try {
            $uploadDir = 'uploads/';
            $imagePath = $uploadDir . basename($imagenFile['name']);
            ...
            $sql = "UPDATE producto SET nombre = :nombre, precio = :precio, descripcion = :descripcion, imagen = :imagen WHERE id_producto = :id";
            $stmt = $this->conn->prepare($sql);
    ...
    public function updateProductCategorie ($id_producto, $categorias){
        try {
            // Verificar que se haya proporcionado un array de categorías
            if (empty($categorias) || !is_array($categorias)) {
                throw new Exception("El array de categorías es inválido o está vacío.");
            ...
            // Construir la consulta SQL
            $sql = "UPDATE producto_categoria SET id_categoria = :id_categoria WHERE id_producto = :id_producto";
    ...
    public function updateProductIngredients($id_producto, $ingredientes){
        try {
            // Verificar que se haya proporcionado un array de ingredientes
            if (empty($ingredientes) || !is_array($ingredientes)) {
                throw new Exception("El array de ingredientes es inválido o está vacío.");
            ...
            // Construir la consulta SQL
            $sql = "UPDATE producto_ingrediente SET id_ingrediente = :id_ingrediente WHERE id_producto = :id_producto";
