...
const AdminOrders = () => {
    ...
    const setOnPreparationOrder = async (id) => {
        try {
            const response = await fetch('http://192.168.0.10:8080/api/updateorderstatus', {
                method: 'PUT',
                headers: {
            ...
            if (result.success) {
                ...
                fetchData('http://192.168.0.10:8080/api/orders/pending', setData);
                fetchData('http://192.168.0.10:8080/api/orders/preparation', setPreparingData);
            } else {
    ...
    const setReadyOrder = async (id) => {
        try {
            ...
            if (result.success) {
                ...
                fetchData('http://192.168.0.10:8080/api/orders/preparation', setPreparingData);
                fetchData('http://192.168.0.10:8080/api/orders/ready', setReadyData);
            } else {
                alert('Error al cambiar el estado del pedido');
            }
        } catch (error) {
    ...
    const HandleShowIngredients = async (id_pedido, id_producto,$id_pedido_producto) => {
        try {
            const response = await fetch(`http://192.168.0.10:8080/api/ingredientsperproduct/${id_pedido}/${id_producto}/${$id_pedido_producto}`);
    ...
    const handleViewDetails = async (id) => {
        try {
            const response = await fetch(`http://192.168.0.10:8080/api/orders/detailorder/${id}`);
            const data = await response.json();
    ...
    const handleTableOrder = async (id) => {
        try {
            const response = await fetch(`http://192.168.0.10:8080/api/tables/perorder/${id}`);
            const data = await response.json();
    ...
    const cancelOrder = async (id) => {
        try {
            const response = await fetch('http://192.168.0.10:8080/api/cancelorder', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
